...
def getGPSData(fileName):
    ...
    for tag in tags.keys():
        if tag == 'GPS GPSTimeStamp':
            t = str(tags[tag]).lstrip('[').rstrip(']').split(',')
            # print values
            ...
            try:
                ...
                altitude_parts = str(tags[tag]).split('/')
                altitude = int(altitude_parts[0]) / int(altitude_parts[1])
...
''' Return Decimal Coordinates if used in command line
    ...
    For Debug Mode:
    python gpsphoto.py "-D" "/path/to/1st/photo" "/path/to/2nd/photo"'''
if len(sys.argv) > 1:
    if sys.argv[0] == "gpsphoto.py":
        if sys.argv[1] == '-D':
            debug = True
        else:
            debug = False
        for i in range(1,len(sys.argv)):
            if debug == True and i >= 2:
                print("\nImage " + str(i - 1) + ":")
